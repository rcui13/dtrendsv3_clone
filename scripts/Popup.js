define([
    ...
    let handleMouseCLK = function (e) {
       console.log(newGlobe.layers)
        let x = e.clientX,
            y = e.clientY;
        console.log(newGlobe.canvasCoordinates(x, y))
       let pickListCLK = newGlobe.pick(newGlobe.canvasCoordinates(x, y));
        console.log(pickListCLK.objects);
        ...
        pickList.objects.forEach(function (value) {
            ...
            if (pickedPM instanceof WorldWind.Placemark) {
                ...
                if (countryCheckbox.checked == true && pickedPM.layer.layerType === 'Country_Placemarks') {
                    ...
                    if (pickedPM.userProperties.Type == "Deaths") {
                        content = "<p style='text-align: center;'> <strong>" + pickedPM.userProperties.dName.replace(/_/g, ' ') + "</strong><br>" + pickedPM.userProperties.Number + " deaths</p>";
                    } else if (pickedPM.userProperties.Type == "Recoveries") {
                        content = "<p style='text-align: center;'> <strong>" + pickedPM.userProperties.dName.replace(/_/g, ' ') + "</strong><br>" + pickedPM.userProperties.Number + " recoveries</p>";
                    } else {
                        content = "<p style='text-align: center;'> <strong>" + pickedPM.userProperties.dName.replace(/_/g, ' ') + "</strong><br>" + pickedPM.userProperties.Number + " cases</p>";
                    }
    ...
    let sitePopUp = function (PM) {
        ...
        if (PM.layer.layerType === "Country_Placemarks") {
            ...
            document.querySelector('#'+ button0.id).dispatchEvent(new CustomEvent('click'));
            ...
            document.querySelector('#plotWeatherButton0').dispatchEvent(new CustomEvent('click'));
        } else {
            //inserts title and discription for placemark
    ...
    //If you don't understand the principle behind this, search "binary search"
